...
class SequenceTaggingNet(nn.Module):
    ...
    def forward(self, x, h0 = None, l = None):
        ...
        # print(f'4. After length processing')
        # print(f'x.shape : {x.shape}')

        x = self.linear(x)
        # x : (batch, hidden_size) -> (batch, 1)

        # print(f'5. After Liner')
        # print(f'x.shape : {x.shape}')

        x = x.squeeze()
        # x : (batch, 1, 1) -> (batch,)
        # print(f'5. After Sqeeze')
        # print(f'x.shape : {x.shape}')

        return x

net = SequenceTaggingNet(train_data.vocab_size + 1, num_layers = 1)
...
def training(model, criterion, optimizer, train_loader, test_loader, epochs = 3, print_every = 40, clip = 5):
...
model, train_losses, test_losses = training(net, criterion, optimizer, train_loader, test_loader, epochs = 2, print_every = 40, clip = 5)
